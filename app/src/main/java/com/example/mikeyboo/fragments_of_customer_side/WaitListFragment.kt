...
import com.example.mikeyboo.adapter.WaitListAdapter
import com.example.mikeyboo.databinding.FragmentWaitListBinding
...
class WaitListFragment : Fragment() {
    ...
    private var list_of_turn_with_wait_list = ArrayList<Turn>()

    private var adapter: WaitListAdapter = WaitListAdapter()

    private val dataManager = DataManager()

    var user: User? = null

    private var _binding: FragmentWaitListBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentWaitListBinding.inflate(inflater, container, false)
    ...
    override fun onViewCreated(view: View, @Nullable savedInstanceState: Bundle?) {
        ...
        list_of_turn_with_wait_list = dataManager.getWaitListToPresent()
        list_of_turn_with_wait_list.sortWith(compareBy<Turn> { it.year }.thenBy { it.month }.thenBy { it.day }.thenBy { it.hours }.thenBy { it.minutes })

        initViews()
    }

    private fun initViews() {
        if (list_of_turn_with_wait_list.isEmpty()) {
            ...
            adapter.WaitAdapter(context, list_of_turn_with_wait_list)
